...
format:
  html:
    ...
    self-contained: true
editor: visual
output-file: "../output/2025/main_analysis_2025.html"
---
...
-   Hard candy has an overall low win percentage, while peanuty or almondy candy and candy that is crisped or has wafers seems to dominate other features regarding win percentage.
...
-   Some interactions are removed. The interactions with the largest positive coefficients are chocolate and peanut/almond, crisped/rice/wafer and bar, and peanut/almond with nougat. The interactions with the largest negative coefficients are peanut/almond and crisped/rice/wafer, peanut/almond and bar, and caramel and peanut/almond.

-   This model can only be seen as robustness check for adding chocolate. Other features are sometimes in contrast to the main regression. This holds even more as we do not have coefficients for every term. In the end, here we see the limits of the small data set.

## 🔍 Cluster analysis

For a better understanding of the main types of candies in the market, we may apply a principal component analysis.
...
fa_parallel <- fa.parallel(df_scaled, fa = "both", n.iter = 100)

# PCA using promax
fit <- principal(df_scaled, nfactors = 3, rotate = "promax", missing = TRUE)
...
-   The scree plot indicates that the data set contains three principal components.

-   The three components can be described as follows:

    1.  Chocolate with caramel and nougat as a bar but not fruity or pluribus

    2.  Hard and fruity with a lot of sugar

    3.  Mostly crisp/rice/wafer witout nougat or peanut/almond
...
-   We see that chocolate, peanut/almond and fruity are still important variables. In addition, bar and hard also seems important.

-   The SHAP value indicate that chocolate has a strong positive influence on popularity, while the effect of peanut/almond and crisped/rice/wafer have the same direction with a lesser magnitude. Fruity is more ambiguous. Still, the result is mostly in line with the main regression.
...
-   Again, the variables chocolate, fruity, peanuty/almondy and crisped/rice/wafer seem to have the most consistent, positive influence.
...
-   Again, chocolate seems is the most important variable. However, in this instance chocolate seems tremendously more important than any other variable. Again, peanuty/almondy and fruity are the top features.

-   The direction and magnitude of the coefficients based on the SHAP values are similar to the main regression at least for chocolate and fruity while peanuty/almondy and crisped/rice/wafer show more mixed results.
